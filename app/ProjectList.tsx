...
function ProjectListItem({ name, logo, active, onClick, last, i }: { name: string, logo: string, active: boolean, onClick: (item: number) => void, last: boolean, i: number }) {
    return <motion.div whileHover={{ scaleY: 1.1 }} whileTap={{ scaleY: 0.9 }} onClick={() => onClick(i)} key={name} style={{ display: "flex", flexDirection: "row", justifyContent: "start", alignItems: active ? "start" : "center" }}>
